...
public class LevelThree {
	...
	public LevelThree(BufferedImage image, Handler handler, Handler handler2, SpriteSheet ss, Camera camera, SpriteSheet enemy_ss, Game game){
	...
	private void loadLevelTwo(BufferedImage level3){
		int w = level3.getWidth();
		int h = level3.getHeight();
		
		for(int xx = 0; xx < w; xx++){
			for(int yy = 0; yy < h; yy++){
				int pixel = level3.getRGB(xx, yy);
				...
				if(green == 255 && blue == 255){
					handler.addObject(new TombStone(xx*32, yy*32, ID.Null, ss, game));
				}
				...
				if(red == 64){
					handler2.addObject(new WaterTile(xx*32, yy*32, ID.WaterTile, ss));
				}
				if(red == 65){
					handler.addObject(new FenceEndRight(xx*32, yy*32, ID.Block, ss));
				}
				if(red == 66){
					handler.addObject(new FenceEndLeft(xx*32, yy*32, ID.Block, ss));
				}
			}
		}
