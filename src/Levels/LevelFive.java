...
public class LevelFive {
	...
	public LevelFive(BufferedImage image, Handler handler, Handler handler2, SpriteSheet ss, Camera camera, SpriteSheet enemy_ss, Game game){
		...
		loadLevelfour(level4);
	}

	private void loadLevelfour(BufferedImage level4){
		int w = level4.getWidth();
		int h = level4.getHeight();
		
		for(int xx = 0; xx < w; xx++){
			for(int yy = 0; yy < h; yy++){
				int pixel = level4.getRGB(xx, yy);
