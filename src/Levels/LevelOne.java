...
public class LevelOne {
	...
	public LevelOne(BufferedImage image, Handler handler, Handler handler2, SpriteSheet ss, Camera camera, SpriteSheet enemy_ss, Game game){
	...
	private void loadLevelOne(BufferedImage level1){
		int w = level1.getWidth();
		int h = level1.getHeight();
		
		for(int xx = 0; xx < w; xx++){
			for(int yy = 0; yy < h; yy++){
				int pixel = level1.getRGB(xx, yy);
