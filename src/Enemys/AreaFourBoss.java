...
public class AreaFourBoss extends GameObject{
	...
	private BufferedImage yeti_boss[] = new BufferedImage[2];
	private int boss_health = 250;
	
	private long startTime = -1;
	private boolean fiveSecondsPast = false;
	private boolean cutscene_finished = false;

	public AreaFourBoss(float x, float y, ID id, Handler handler, Handler handler2, Game game, SpriteSheet enemy_ss) {
		...
		yeti_boss[0] = enemy_ss.grabImage(3, 14, 96, 128);
		yeti_boss[1] = enemy_ss.grabImage(6, 14, 96, 128);
		...
		anim = new Animation(6, yeti_boss[0], yeti_boss[1]);
	}
	...
	public void tick() {
		...
		if(cutscene_finished == true){
			...
			if(timePast >= 2000 && fiveSecondsPast == false){
				for(int i = 0; i < 10; i++){
					handler.addObject(new BallAttack(x, y, ID.WaterBall, game, handler, enemy_ss, 2, 1));
				}
	...
	public void render(Graphics g) {
		...
		g.fillRect((int)getX()  - 80, (int)getY() - 21, 250 + 4, 14);
		g.setColor(Color.GREEN);
		g.fillRect((int)getX() - 78, (int)getY() - 19, boss_health, 10);
	}
