...
public class YetiSpecial extends GameObject{
	...
	private long rageTime = -1;
	private boolean fiveSecondsPast = false;
	
	private int yetiHP = 100;

	public YetiSpecial(float x, float y, ID id, Handler handler, Handler handler2, Game game, SpriteSheet enemy_ss) {
		...
		yeti_image[0] = enemy_ss.grabImage(1, 16, 32, 64);
		yeti_image[1] = enemy_ss.grabImage(2, 16, 32, 64);
	...
	public void tick() {
		...
		velY = (float) ((speedChange / distance * diffY)); // make spaz out if player is dead and zombies are at players x
		...
		if(timePast >= 4000 && fiveSecondsPast == false){
			anim.img1 = yeti_image[0] = enemy_ss.grabImage(1, 18, 32, 64);
			anim.img2 = yeti_image[1] = enemy_ss.grabImage(2, 18, 32, 64);
			handler.addObject(new BallAttack(x, y, ID.WaterBall, game, handler, enemy_ss, 2, 0));
			...
			anim.img1 = yeti_image[0] = enemy_ss.grabImage(1, 16, 32, 64);
			anim.img2 = yeti_image[1] = enemy_ss.grabImage(2, 16, 32, 64);
		...
		if(fiveSecondsPast == true){
			if(rageTime == -1){
				rageTime = System.currentTimeMillis();
			}
			
			timePast = System.currentTimeMillis() - rageTime;
