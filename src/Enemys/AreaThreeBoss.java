...
public class AreaThreeBoss extends GameObject{
	
	private BufferedImage chomper_boss[] = new BufferedImage[2];
	...
	public AreaThreeBoss(float x, float y, ID id, Handler handler, Handler handler2, Game game, SpriteSheet enemy_ss) {
		...
		chomper_boss[0] = enemy_ss.grabImage(5, 11, 128, 96);
		chomper_boss[1] = enemy_ss.grabImage(9, 11, 128, 96);
		...
		anim = new Animation(6, chomper_boss[0], chomper_boss[1]);
	}
	...
	public void tick() {
		...
		if(cutscene_finished == true){
			...
			if(timePast >= 2000 && fiveSecondsPast == false){
				handler.addObject(new AcidPoolBall(x + 43, y + 38, ID.AcidPoolBall, game, handler, enemy_ss));
				startTime = -1;
			}
	...
	public void render(Graphics g) {
		...
		g.fillRect((int)getX()  - 30, (int)getY() - 21, 200 + 4, 14);
		g.setColor(Color.GREEN);
		g.fillRect((int)getX() - 28, (int)getY() - 19, boss_health, 10);
	}

	public Rectangle getBounds() {
		return new Rectangle((int)x, (int)y, 128, 96);
	}
}
